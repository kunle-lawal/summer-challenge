...
const SaveBtn = styled.button<{ $saved?: boolean }>`
  background: ${({ theme, $saved }) => ($saved ? theme.color.greenDim : theme.color.gold)};
  color: ${({ theme, $saved }) => ($saved ? theme.color.green : theme.color.bg)};
  border: ${({ $saved }) => ($saved ? '1px solid rgba(74,222,128,0.3)' : 'none')};
  border-radius: ${({ theme }) => theme.radii.md};
  padding: 9px 0;
  width: 100%;
  font-family: ${({ theme }) => theme.font.display};
...
export function PersonLogCard({
  ...
  const alreadySaved = entries.some((e) => e.date === date && e.personId === person.id);
  const savedEntry =
    entries.find((e) => e.date === date && e.personId === person.id) ?? null;

  const hasValues = Boolean(formRow.gym && formRow.junk);
  const livePts = hasValues ? calcPts(formRow.gym, formRow.steps, formRow.junk) : null;
  const dayWorkoutPts =
    alreadySaved && savedEntry ? savedEntry.pts : livePts;

  const profileToday = profiles[person.id]?.entries?.find((e) => e.date === date);
  const profilePts = profileToday ? profileToday.pts : null;
    ...
    <Card $saved={alreadySaved}>
      <div>
        ...
        <PersonFree>
          ...
          {profilePts != null && (
            <>
              <br />
              <GoalPtsLine>Goal: +{profilePts}pts</GoalPtsLine>
            </>
          )}
        </PersonFree>
      </div>
      ...
      <div>
        ...
        >
          ...
          <option value="went" disabled={gymWentDisabled}>
            {gymWentLabel}
          </option>
          <option value="skip">✗ Skipped (0)</option>
          <option value="free-gym" disabled={gymFreeDisabled}>
            {gymFreeLabel}
          </option>
        </FieldSelect>
        {wl.gymMaxed ? <WindowWarn>4/4 gym days used this week</WindowWarn> : null}
      </div>
      ...
      <div>
        ...
        >
          <option value="">— select —</option>
          <option value="clean" disabled={junkCleanDisabled}>
            {junkCleanLabel}
          </option>
          <option value="ate">✗ Ate Junk (−1)</option>
          <option value="free-junk" disabled={junkFreeDisabled}>
            {junkFreeLabel}
          </option>
        </FieldSelect>
        {wl.cleanMaxed ? <WindowWarn>6/6 clean days used this week</WindowWarn> : null}
      </div>
      ...
      <SaveCol>
        {alreadySaved ? (
          <SaveBtn $saved disabled>
            Saved ✓
          </SaveBtn>
        ) : (
          <SaveBtn type="button" onClick={onClickSave}>
            Save Day
          </SaveBtn>
        )}
      </SaveCol>

      {validationError ? <ValidationMsg>{validationError}</ValidationMsg> : null}
      {showConfirm ? (
        <ConfirmBar>
          <span>Once saved you can&apos;t update today&apos;s entry. Lock it in?</span>
          <ConfirmBtns>
            <BtnGhost type="button" onClick={onCancelConfirm}>
              Cancel
            </BtnGhost>
            <BtnGold type="button" onClick={onConfirmSave}>
              Lock In
            </BtnGold>
          </ConfirmBtns>
        </ConfirmBar>
