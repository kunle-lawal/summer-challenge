...
- **Save:** Builds `WorkoutEntry` with **`date = logDate`**, **`pts` from `calcPtsForLogDay`**, merges into `entries` via `updateCache`, `postToSheets('saveWorkout', entry)`.
