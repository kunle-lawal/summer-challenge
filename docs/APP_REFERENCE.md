...
└── src/
    ...
    ├── components/         # layout/, log/, profile/ (+ personalGoalHorizontalStyles)
...
| Component | File | Role |
|-----------|------|------|
| `PersonLogCard` | [`PersonLogCard.tsx`](../src/components/log/PersonLogCard.tsx) | One person’s gym/steps/junk, points, save + confirm bar; styled-components colocated here. |
| `PersonalGoalPanel` | [`PersonalGoalPanel.tsx`](../src/components/profile/PersonalGoalPanel.tsx) | Single-person personal goal UI in a **horizontal grid** (same column pattern as **`PersonLogCard`**); uses [`personalGoalHorizontalStyles.ts`](../src/components/profile/personalGoalHorizontalStyles.ts) + unlock/confirm bits from **`profilesStyles.ts`**. |
...
- **Local state:** one `LogFormRow` (`gym`, `steps`, `junk`), confirm flag, validation message — for the **selected person only**.
- **Renders:** that person’s **`PersonLogCard`**, then **`PersonalGoalPanel`** (personal goal no longer has its own route).
- **Resets form** when `pathname === '/log'`, when **`clearGeneration`** changes, or when **`person.id`** changes.
- **Save:** Builds `WorkoutEntry`, merges into `entries` via `updateCache`, `postToSheets('saveWorkout', entry)`.
...
- Builds rows via **`buildHistoryRows`** (`lib/history.ts`), sorts like legacy (date desc, then time).
...
|--------|------|------------------|
| Config | [`config.ts`](../src/lib/config.ts) | Reads **`import.meta.env`**, `PEOPLE` JSON parse with defaults, `SCRIPT_URL`, `CHALLENGE_START`, goal-reset date bounds, `FREE_LIMIT`, `PROFILE_MAX_PTS`, `isScriptConfigured()`. |
...
| Dates | [`dates.ts`](../src/lib/dates.ts) | `today`, `todayDisplay`, `normalizeDateToYYYYMMDD` (Sheets serial/ISO), `dayIndex` / `windowOf`, history display formatters. |
| Scoring | [`scoring.ts`](../src/lib/scoring.ts) | `calcPts`, `calcProfilePts`, `getWindowLimits`, `getFreeCounts`, `getTotals`, `initials`, `fmtPts`, `ptsClass`, `stepsPtsFromEntry`. |
...
| History rows | [`history.ts`](../src/lib/history.ts) | `buildHistoryRows` → discriminated **`HistoryDisplayRow`** for the History table (workout vs goal vs goal_set vs goal_reset). |
